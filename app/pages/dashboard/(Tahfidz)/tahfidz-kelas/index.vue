<script setup lang="ts">
  ...
  const tahfidzEdit = authStore.hasPermission({
    tahfidz: ["manage"],
  });
  ...
  const { data, status, refresh } = await useFetch(`${APIBASE}/kelas-tahfidz`, {
    query,
  });
  ...
  async function onSubmit(event: FormSubmitEvent<Schema>) {
    const basePath = `${APIBASE}/kelas-tahfidz`;
    await execute({
  ...
  async function clickDelete(ids: number[]) {
    openConfirmModal("/kelas-tahfidz", { id: ids }, refresh);
  }
...
<template>
  <Title>Tahfidz | Daftar Kelas</Title>
  <main>
    <LazyUModal
      ...
      <template #body>
        ...
        >
          <UFormField label="Kelas Pengajian" name="nama">
            <UInput
              v-model="state.nama"
              :disabled="isLoading || !tahfidzEdit || viewStatus"
            />
          </UFormField>
    ...
    <UCard>
      <div class="mb-4 flex gap-2 md:mb-6 md:gap-4">
        ...
        />
        <AppTambahExport
          :add-permission="tahfidzEdit"
          :add-function="clickAdd"
          path="kelas-tahfidz/export"
        />
      </div>
      <AppTable
        ...
        enumerate
        :deletable="tahfidzEdit"
        :editable="tahfidzEdit"
        viewable
        selectable
        pagination
        @edit="clickUpdate"
