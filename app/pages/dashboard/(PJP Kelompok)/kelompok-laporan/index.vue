<script setup lang="ts">
  ...
  constantStore.setTitle("PJP Kelompok / Laporan Muslimun");

  const musyId = ref<number | undefined>();
  const { data: musyOption, status: statusMusy } = await useFetch(
    `${APIBASE}/options/muslimun`
  );

  const { data, status, refresh } = await useFetch(
    `${APIBASE}/muslimun/laporan`,
    {
      query: {
        musyawarahId: musyId,
      },
  ...
  async function onSubmit() {
    const basePath = `${APIBASE}/muslimun/laporan`;
    await execute({
...
<template>
  <Title>PJP Kelompok | Laporan Muslimun</Title>
  <LazyUModal
    v-model:open="modalOpen"
    title="Detail Laporan Muslimun"
    class="max-w-4xl"
  >
    <template #body>
