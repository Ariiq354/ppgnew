<script setup lang="ts">
  ...
  constantStore.setTitle("PJP Kelompok / Daftar Muslimun");
  ...
  const { data, status, refresh } = await useFetch(`${APIBASE}/muslimun`, {
    query,
  });
  ...
  async function onSubmit(event: FormSubmitEvent<Schema>) {
    const basePath = `${APIBASE}/muslimun`;
    await execute({
  ...
  async function clickDelete(ids: number[]) {
    openConfirmModal("/muslimun", { id: ids }, refresh);
  }
...
<template>
  <Title>PJP Kelompok | Daftar Muslimun</Title>
  <main>
    <LazyUModal
      v-model:open="modalOpen"
      :title="
        (state.id ? (viewStatus ? 'Detail' : 'Edit') : 'Tambah') + ' Muslimun'
      "
      class="max-w-2xl"
    >
      <template #body>
        <UForm
          ...
          @submit="onSubmit"
        >
          <UFormField label="Nama Muslimun" name="nama">
            <UInput
              v-model="state.nama"
