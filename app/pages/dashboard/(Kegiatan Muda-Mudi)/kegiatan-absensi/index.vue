<script setup lang="ts">
  ...
  const { data: kelasOption, status: statusKelas } = await useFetch(
    `${APIBASE}/options/kelas-mudamudi`,
    {
      query: {
        nama: namaKelas,
      },
  ...
  const {
    ...
    refresh,
  } = await useFetch(() => `${APIBASE}/absensi-mudamudi/${kelasId.value}`, {
    immediate: false,
    watch: false,
  });
  watch(kelasId, () => {
  ...
  const { data, status } = await useFetch(
    `${APIBASE}/absensi-mudamudi/generus`,
    {
      query,
    }
  );
...
<template>
  <Title>Kegiatan & Muda-mudi | Absensi Generus</Title>
