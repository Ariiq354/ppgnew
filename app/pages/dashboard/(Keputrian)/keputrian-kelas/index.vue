<script setup lang="ts">
  ...
  const keputrianEdit = authStore.hasPermission({
    keputrian: ["manage"],
  });
  constantStore.setTitle("Keputrian / Daftar Kelas");
  ...
  async function onSubmit(event: FormSubmitEvent<Schema>) {
    const basePath = `${APIBASE}/kelas-keputrian`;
    await execute({
  ...
  async function clickDelete(ids: number[]) {
    openConfirmModal("/kelas-keputrian", { id: ids }, refresh);
  }
...
<template>
  <Title>Keputrian | Daftar Kelas</Title>
  <main>
    <LazyUModal
    ...
    <UCard>
      <div class="mb-4 flex gap-2 md:mb-6 md:gap-4">
        ...
        />
        <AppTambahExport
          :add-permission="keputrianEdit"
          :add-function="clickAdd"
          path="kelas-keputrian/export"
        />
      </div>
      <AppTable
        ...
        enumerate
        :deletable="keputrianEdit"
        :editable="keputrianEdit"
        viewable
        selectable
        pagination
        @edit="clickUpdate"
