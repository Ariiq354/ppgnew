<script setup lang="ts">
  ...
  const kurikulumManage = authStore.hasPermission({
    kurikulum: ["manage"],
  });
  constantStore.setTitle("Kurikulum / Laporan Bidang");
  ...
  async function clickDelete(ids: number[]) {
    openConfirmModal(
      ...
      { id: ids, musyawarahId: musyId, bidang: "kurikulum" },
      refresh
    );
  }
...
<template>
  <Title>Kurikulum | Laporan Bidang</Title>
  <LazyUModal
  ...
  <main class="grid grid-cols-1 gap-4 md:grid-cols-3">
    <UCard v-if="kurikulumManage" class="md:col-span-2">
    ...
    <UCard v-if="!kurikulumManage" class="md:col-span-3">
      <h1 class="text-2xl font-bold sm:text-3xl">Hasil Musyawarah</h1>
      <p class="text-muted mb-8 text-sm sm:text-base">
        Pencatatan rangkuman hasil musyawarah
      </p>
    ...
    <UCard class="h-fit" :class="{ 'md:col-span-3': !kurikulumManage }">
      ...
      <div v-else class="space-y-4">
        ...
        >
          ...
          <UTooltip v-if="kurikulumManage" text="Hapus">
            <UButton
