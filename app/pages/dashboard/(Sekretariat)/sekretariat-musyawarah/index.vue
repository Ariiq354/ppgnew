<script setup lang="ts">
  ...
  const sekretariatEdit = authStore.hasPermission({
    sekretariat: ["manage"],
  });
  ...
  const { data, status, refresh } = await useFetch(`${APIBASE}/musyawarah`, {
    query,
  });
  ...
  async function onSubmit(event: FormSubmitEvent<Schema>) {
    const basePath = `${APIBASE}/musyawarah`;
    await execute({
  ...
  async function clickDelete(ids: number[]) {
    openConfirmModal("/musyawarah", { id: ids }, refresh);
  }
  ...
  <main>
      ...
      <template #body>
        ...
        >
          <UFormField label="Nama Musyawarah" name="nama">
            <UInput
              v-model="state.nama"
              :disabled="isLoading || !sekretariatEdit || viewStatus"
            />
          </UFormField>
    ...
    <UCard>
      ...
      <AppTable
        ...
        enumerate
        :deletable="sekretariatEdit"
        :editable="sekretariatEdit"
        viewable
        selectable
        pagination
        @edit="clickUpdate"
